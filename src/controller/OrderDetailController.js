...
exports.getAllOrderDetailVertical_1 = async (req,res) => {
    try {
        let dataStorage = await orderDetail.getAllOrderDetailVertical_1();
        res.status(200).json(dataStorage);
    } catch (e) {
        console.error(e);
        res.status(500).json({ error: `Database error: ${e.message}` });
...
exports.getAllOrderDetailVertical_2 = async (req,res) => {
    try {
        let dataStorage = await orderDetail.getAllOrderDetailVertical_2();
        res.status(200).json(dataStorage);
    } catch (e) {
        console.error(e);
        res.status(500).json({ error: `Database error: ${e.message}` });
